...
public class RemadeChatMonitor : IChatMonitor
{
	...
	public void NewText(string newText, byte R = byte.MaxValue, byte G = byte.MaxValue, byte B = byte.MaxValue)
	{
		...
		AddNewMessage(newText, new Color((int)R, (int)G, (int)B));
	}
	...
	public void DrawChat(bool drawingPlayerChat)
	{
		...
		while (num5 < _showCount && num2 < _messages.Count)
		{
			...
			ChatManager.DrawColorCodedStringWithShadow(Main.spriteBatch, FontAssets.MouseText.Value, snippetWithInversedIndex, new Vector2(88f, (float)(Main.screenHeight - 30 - 28 - num5 * 21)), 0f, Vector2.Zero, Vector2.One, out hoveredSnippet);
			if (hoveredSnippet >= 0)
			{
				num7 = hoveredSnippet;
				num6 = num2;
