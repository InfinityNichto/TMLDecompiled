...
public class BuffDefinition : EntityDefinition
{
	...
	public override int Type
	{
		get
		{
			...
			if (!BuffID.Search.TryGetId((Mod != "Terraria") ? (Mod + "/" + Name) : Name, out var id))
			{
				return -1;
			}
			return id;
	...
	public BuffDefinition(int type)
		: base(BuffID.Search.GetName(type))
	{
	}

	public BuffDefinition(string key)
		: base(key)
	{
	}

	public BuffDefinition(string mod, string name)
		: base(mod, name)
	{
	}
