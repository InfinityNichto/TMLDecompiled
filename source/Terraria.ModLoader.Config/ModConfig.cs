...
public abstract class ModConfig : ILocalizedModType, IModType
{
	...
	[JsonIgnore]
	public virtual LocalizedText DisplayName => Language.GetOrRegister(this.GetLocalizationKey("DisplayName"), () => ConfigManager.GetLegacyLabelAttribute(GetType())?.LocalizationEntry ?? Regex.Replace(Name, "([A-Z])", " $1").Trim());
