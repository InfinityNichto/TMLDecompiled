...
public sealed class TEHatRack : TileEntity, IFixLoadedData
{
	...
	public static void Framing_CheckTile(int callX, int callY)
	{
		...
		Kill(num, num2);
		if (TileLoader.Drop(callX, callY, 475))
		{
	...
	public void Draw(int tileLeftX, int tileTopY)
	{
		...
		dollPlayer.direction *= -1;
		Vector2 vector2 = new Vector2((float)(-dollPlayer.width / 2), (float)(-dollPlayer.height - 6)) + new Vector2((float)(dollPlayer.direction * 14), -2f);
		...
		dollPlayer.direction *= -1;
		vector2 = new Vector2((float)(-dollPlayer.width / 2), (float)(-dollPlayer.height - 6)) + new Vector2((float)(dollPlayer.direction * 12), 16f);
		dollPlayer.position = vector + vector2;
		Main.PlayerRenderer.DrawPlayer(Main.Camera, dollPlayer, dollPlayer.position, 0f, dollPlayer.fullRotationOrigin);
	}
	...
	private void DrawSlotPairSet(Player player, SpriteBatch spriteBatch, int slotsToShowLine, int slotsArrayOffset, float offsetX, float offsetY, int inventoryContextTarget)
	{
		...
		for (int i = 0; i < slotsToShowLine; i++)
		{
			for (int j = 0; j < 2; j++)
			{
				...
				ItemSlot.Draw(spriteBatch, items, num, i + slotsArrayOffset, new Vector2((float)num2, (float)num3));
			}
		}
	}
	...
	public override void NetSend(BinaryWriter writer)
	{
		...
		{
			itemsBits[k] = !_items[k].IsAir;
			itemsBits[k + _items.Length] = !_dyes[k].IsAir;
		}
		...
		for (int i = 0; i < _dyes.Length; i++)
		{
			...
			if (!dye.IsAir)
			{
				ItemIO.Send(dye, writer, writeStack: true);
			}
		}
	}
	...
	public override void NetReceive(BinaryReader reader)
	{
		...
		for (int j = 0; j < _items.Length; j++)
		{
			_items[j] = (presentItems[j] ? ItemIO.Receive(reader, readStack: true) : new Item());
		}
		for (int i = 0; i < _dyes.Length; i++)
		{
			_dyes[i] = (presentItems[i + _items.Length] ? ItemIO.Receive(reader, readStack: true) : new Item());
		}
	}
}
