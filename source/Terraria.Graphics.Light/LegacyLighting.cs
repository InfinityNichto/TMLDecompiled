...
public class LegacyLighting : ILightingEngine
{
	...
	public void ProcessArea(Rectangle area)
	{
		...
		float num = (float)(int)((Color)(ref Main.tileColor)).R / 255f;
		float num2 = (float)(int)((Color)(ref Main.tileColor)).G / 255f;
		float num3 = (float)(int)((Color)(ref Main.tileColor)).B / 255f;
		...
		_requestedRectLeft = ((Rectangle)(ref area)).Left;
		_requestedRectRight = ((Rectangle)(ref area)).Right;
		_requestedRectTop = ((Rectangle)(ref area)).Top;
		_requestedRectBottom = ((Rectangle)(ref area)).Bottom;
	...
	private void TryUpdatingMapWithLight()
	{
		...
		Vector2 val = unscaledPosition + Main.GameViewMatrix.Translation;
		int value = (int)Math.Floor(val.X / 16f);
		int value2 = (int)Math.Floor((val.X + x) / 16f) + 1;
		int value3 = (int)Math.Floor(val.Y / 16f);
		int value4 = (int)Math.Floor((val.Y + y) / 16f) + 1;
	...
	private void CopyFullyProcessedDataOver(int maxLightArrayX, int maxLightArrayY)
	{
		...
		int num3 = (int)Math.Floor(unscaledPosition.Y / 16f) - _scrY;
		if (num > 16)
		{
			num = 0;
		}
		...
		int num4 = 0;
		int num5 = maxLightArrayX;
		int num6 = 0;
		int num7 = maxLightArrayY;
		if (num < 0)
		{
			num4 -= num;
		}
		...
		if (_rgb)
		{
			int num8 = maxLightArrayX;
			if (_states.Length <= num8 + num)
			{
				num8 = _states.Length - num - 1;
			}
			for (int i = num4; i < num8; i++)
			{
				...
				int num9 = num7;
				if (array2.Length <= num9 + num)
				{
					num9 = array2.Length - num3 - 1;
				}
				for (int j = num6; j < num9; j++)
				{
					LightingState obj = array[j];
					LightingState lightingState = array2[j + num3];
		...
		int num10 = num5;
		if (_states.Length <= num10 + num)
		{
			num10 = _states.Length - num - 1;
		}
		for (int k = num4; k < num10; k++)
		{
			LightingState[] array3 = _states[k];
			LightingState[] array4 = _states[k + num];
			int num2 = num7;
			if (array4.Length <= num2 + num)
			{
				num2 = array4.Length - num3 - 1;
			}
			for (int l = num6; l < num2; l++)
			{
				LightingState obj2 = array3[l];
				LightingState lightingState2 = array4[l + num3];
	...
	private void PreRenderPhase()
	{
		...
		_ = (float)(int)((Color)(ref Main.tileColor)).R / 255f;
		_ = (float)(int)((Color)(ref Main.tileColor)).G / 255f;
		_ = (float)(int)((Color)(ref Main.tileColor)).B / 255f;
		Stopwatch stopwatch = new Stopwatch();
		stopwatch.Start();
		int num = 0;
		int num8 = (int)_camera.UnscaledSize.X / 16 + Lighting.OffScreenTiles * 2 + 10;
		int num9 = 0;
		int num10 = (int)_camera.UnscaledSize.Y / 16 + Lighting.OffScreenTiles * 2 + 10;
		_minX = num8;
		_maxX = num;
		_minY = num10;
		_maxY = num9;
		_rgb = Mode == 0 || Mode == 3;
		for (int i = num; i < num8; i++)
		{
			LightingState[] array = _states[i];
		...
		if (Main.wofNPCIndex >= 0 && Main.player[Main.myPlayer].gross)
		{
			try
			{
				int num11 = (int)_camera.UnscaledPosition.Y / 16 - 10;
				int num12 = (int)(_camera.UnscaledPosition.Y + _camera.UnscaledSize.Y) / 16 + 10;
				int num13 = (int)Main.npc[Main.wofNPCIndex].position.X / 16;
				num13 = ((Main.npc[Main.wofNPCIndex].direction <= 0) ? (num13 + 2) : (num13 - 3));
				int num14 = num13 + 8;
				float num15 = 0.5f * Main.demonTorch + 1f * (1f - Main.demonTorch);
				float num2 = 0.3f;
				float num3 = 1f * Main.demonTorch + 0.5f * (1f - Main.demonTorch);
				num15 *= 0.2f;
				num2 *= 0.1f;
				num3 *= 0.3f;
				for (int k = num13; k <= num14; k++)
				{
					LightingState[] array2 = _states[k - num13];
					for (int l = num11; l <= num12; l++)
					{
						LightingState lightingState = array2[l - _expandedRectTop];
						if (lightingState.R2 < num15)
						{
							lightingState.R2 = num15;
						}
						if (lightingState.G2 < num2)
						{
							lightingState.G2 = num2;
						}
						if (lightingState.B2 < num3)
						{
							lightingState.B2 = num3;
						}
					}
				}
			}
		...
		num8 = Utils.Clamp(_expandedRectRight, 5, Main.maxTilesX - 1);
		num9 = Utils.Clamp(_expandedRectTop, 5, Main.maxTilesY - 1);
		num10 = Utils.Clamp(_expandedRectBottom, 5, Main.maxTilesY - 1);
		Main.SceneMetrics.ScanAndExportToMain(new SceneMetricsScanSettings
		{
			VisualScanArea = new Rectangle(num, num9, num8 - num, num10 - num9),
			BiomeScanCenterPositionInWorld = Main.LocalPlayer.Center,
			ScanOreFinderData = Main.LocalPlayer.accOreFinder
		});
		_tileScanner.Update();
		_tileScanner.ExportTo(new Rectangle(num, num9, num8 - num, num10 - num9), _lightMap, new TileLightScannerOptions
		{
		...
		for (int m = num; m < num8; m++)
		{
			...
			for (int n = num9; n < num10; n++)
			{
				...
				_lightMap.GetLight(m - num, n - num9, out var color);
				if (_rgb)
				{
				...
				switch (_lightMap.GetMask(m - num, n - num9))
				{
				...
				{
					int num4 = m - _expandedRectLeft;
					int num5 = n - _expandedRectTop;
					if (_minX > num4)
					{
						_minX = num4;
					}
		...
		{
			int num6 = tempLight.Key.X - _requestedRectLeft + Lighting.OffScreenTiles;
			int num7 = tempLight.Key.Y - _requestedRectTop + Lighting.OffScreenTiles;
			if (num6 >= 0 && (float)num6 < _camera.UnscaledSize.X / 16f + (float)(Lighting.OffScreenTiles * 2) + 10f && num7 >= 0 && (float)num7 < _camera.UnscaledSize.Y / 16f + (float)(Lighting.OffScreenTiles * 2) + 10f)
			{
				LightingState lightingState3 = _states[num6][num7];
			...
			while (true)
			{
				...
				for (int i = outerLoopStart; i < outerLoopEnd; i++)
				{
					...
					for (int j = num5; j != num6; j += num)
					{
						...
						{
							num4 = (lightingState.StopLight ? (num4 * _negLight2) : ((!lightingState.WetLight) ? (num4 * _negLight) : ((!lightingState.HoneyLight) ? (num4 * (_wetLightB * (float)fastRandom.Next(97, 100) * 0.01f)) : (num4 * (_honeyLightB * (float)fastRandom.Next(97, 100) * 0.01f)))));
						}
					}
				}
				if (flag)
