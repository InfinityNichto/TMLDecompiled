...
public class CustomCurrencyManager
{
	...
	public static void DrawSavings(SpriteBatch sb, int currencyIndex, float shopx, float shopy, bool horizontal = false)
	{
		...
		if (num5 > 0)
		{
			...
			if (num4 > 0)
			{
				sb.Draw(itemTexture, Utils.CenteredRectangle(new Vector2(shopx + 96f, shopy + 50f), itemFrame.Size() * 0.65f), (Rectangle?)null, Color.White);
			}
			if (num3 > 0)
			{
				sb.Draw(itemTexture2, Utils.CenteredRectangle(new Vector2(shopx + 80f, shopy + 50f), itemFrame2.Size() * 0.65f), (Rectangle?)null, Color.White);
			}
			if (num2 > 0)
			{
				sb.Draw(itemTexture3, Utils.CenteredRectangle(new Vector2(shopx + 80f, shopy + 50f), itemFrame3.Size() * 0.65f), (Rectangle?)null, Color.White);
			}
			if (num > 0)
			{
				sb.Draw(itemTexture4, Utils.CenteredRectangle(new Vector2(shopx + 70f, shopy + 60f), itemFrame4.Size() * 0.65f), (Rectangle?)null, Color.White);
			}
	...
	{
		return _currencies.TryGetValue(id, out system);
	}
}
