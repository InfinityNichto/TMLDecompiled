...
public class NetClientSocialModule : NetSocialModule
{
	...
	public override void Initialize()
	{
		...
		{
			Terraria.ModLoader.ModLoader.OnSuccessfulLoad = (Action)Delegate.Combine(Terraria.ModLoader.ModLoader.OnSuccessfulLoad, new Action(CheckParameters));
		}
	}

	private void CheckParameters()
	{
		if (ulong.TryParse(Program.LaunchParameters["+connect_lobby"], out var result))
		{
			ConnectToLobby(result);
			return;
		}
		Logging.tML.Error((object)("The provided lobby ID was invalid: " + result));
		Main.menuMode = 0;
	}
	...
	private void OnLobbyJoinRequest(GameLobbyJoinRequested_t result)
	{
		...
		Main.QueueMainThreadAction(delegate
		{
			...
			if (Terraria.ModLoader.ModLoader.Mods.Length == 0 || Main.menuMode == 10002)
			{
				Logging.tML.Info((object)"OnLobbyJoinRequest: Delay joinAction via OnSuccessfulLoad");
				Terraria.ModLoader.ModLoader.OnSuccessfulLoad = (Action)Delegate.Combine(Terraria.ModLoader.ModLoader.OnSuccessfulLoad, action);
			}
			else
			{
				Logging.tML.Info((object)"OnLobbyJoinRequest: Running joinAction directly");
				action();
			}
		});
	}
	...
	private void OnLobbyEntered(LobbyEnter_t result, bool failure)
	{
		...
		while (SteamNetworking.GetP2PSessionState(_lobby.Owner, ref pConnectionState) && pConnectionState.m_bConnectionActive != 1)
		{
