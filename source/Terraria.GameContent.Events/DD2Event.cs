...
public class DD2Event
{
	...
	public static void SummonCrystalDirect(int x, int y, int whoAsks)
	{
		...
		if (!NPC.AnyNPCs(548))
		{
			...
			if (tileSafely.active() && tileSafely.type == 466)
			{
				...
				((Point)(ref point))._002Ector(x * 16, y * 16);
	...
	public static bool WouldFailSpawningHere(int x, int y)
	{
		...
		StrayMethods.CheckArenaScore(Utils.ToWorldCoordinates(new Point(x, y), 8f, 8f), out var xLeftEnd, out var xRightEnd);
	...
	public static void FailureMessage(int client)
	{
		...
		((Color)(ref color))._002Ector(255, 255, 0);
		if (Main.netMode == 2)
		{
		...
		{
			Main.NewText(text.Value, ((Color)(ref color)).R, ((Color)(ref color)).G, ((Color)(ref color)).B);
		}
	}
	...
	public static void FindArenaHitbox()
	{
		...
		((Vector2)(ref vector))._002Ector(float.MaxValue, float.MaxValue);
		Vector2 vector2 = default(Vector2);
		((Vector2)(ref vector2))._002Ector(0f, 0f);
		for (int i = 0; i < 200; i++)
		{
			NPC nPC = Main.npc[i];
	...
	public static bool ShouldBlockBuilding(Vector2 worldPosition)
	{
		...
		//IL_0006: Unknown result type (might be due to invalid IL or missing references)
		return ((Rectangle)(ref ArenaHitbox)).Contains(worldPosition.ToPoint());
	}
	...
	private static void SummonBetsy()
	{
		...
		if (!_spawnedBetsyT3 && !NPC.AnyNPCs(551))
		{
			...
			((Vector2)(ref position))._002Ector(1f, 1f);
			int num = NPC.FindFirstNPC(548);
			if (num != -1)
			{
	...
	public static bool IsStandActive(int x, int y)
	{
		...
		((Vector2)(ref target))._002Ector((float)(x * 16 + 8), (float)(y * 16 + 8));
		for (int i = 0; i < 200; i++)
		{
			NPC nPC = Main.npc[i];
