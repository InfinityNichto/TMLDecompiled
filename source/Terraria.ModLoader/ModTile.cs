...
public abstract class ModTile : ModBlockType
{
	...
	public bool IsDoor
	{
		get
		{
			if (TileID.Sets.OpenDoorID[base.Type] == -1)
			{
				return TileID.Sets.CloseDoorID[base.Type] != -1;
			}
			return true;
		}
	}
	...
	public void AddToArray(ref int[] array)
	{
		...
		array[array.Length - 1] = base.Type;
	}
	...
	public void RegisterItemDrop(int itemType, params int[] tileStyles)
	{
		...
		{
			TileLoader.tileTypeAndTileStyleToItemType[(base.Type, -1)] = itemType;
			return;
		}
		foreach (int tileStyle in tileStyles)
		{
			TileLoader.tileTypeAndTileStyleToItemType[(base.Type, tileStyle)] = itemType;
		}
	}
	...
	{
		TextureAssets.Tile[base.Type] = ModContent.Request<Texture2D>(Texture);
		SetStaticDefaults();
		if (TileObjectData.newTile.Width > 1 || TileObjectData.newTile.Height > 1)
		{
			TileObjectData.FixNewTile();
			throw new Exception("It appears that you have an error surrounding TileObjectData.AddTile in " + GetType().FullName)
			{
				HelpLink = "https://github.com/tModLoader/tModLoader/wiki/Basic-tModLoader-Modding-FAQ#tileobjectdataaddtile-issues"
			};
		}
		if (Main.tileLavaDeath[base.Type])
		{
			Main.tileObsidianKill[base.Type] = true;
		}
		if (Main.tileSolid[base.Type])
		{
			Main.tileNoSunLight[base.Type] = true;
		}
		PostSetDefaults();
		if (TileID.Sets.HasOutlines[base.Type])
		{
			TextureAssets.HighlightMask[base.Type] = ModContent.Request<Texture2D>(HighlightTexture);
		}
		TileID.Search.Add(base.FullName, base.Type);
	}
