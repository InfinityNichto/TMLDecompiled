...
public abstract class GlobalNPC : GlobalType<NPC, GlobalNPC>
{
	...
	{
		base.ValidateType();
		LoaderUtils.MustOverrideTogether(this, (GlobalNPC g) => new Action<NPC, TagCompound>(g.SaveData), (GlobalNPC g) => new Action<NPC, TagCompound>(g.LoadData));
		LoaderUtils.MustOverrideTogether(this, (GlobalNPC g) => new Action<NPC, BitWriter, BinaryWriter>(g.SendExtraAI), (GlobalNPC g) => new Action<NPC, BitReader, BinaryReader>(g.ReceiveExtraAI));
	}
