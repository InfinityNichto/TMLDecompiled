...
public abstract class GlobalItem : GlobalType<Item, GlobalItem>
{
	...
	{
		base.ValidateType();
		LoaderUtils.MustOverrideTogether(this, (GlobalItem g) => new Action<Item, TagCompound>(g.SaveData), (GlobalItem g) => new Action<Item, TagCompound>(g.LoadData));
		LoaderUtils.MustOverrideTogether(this, (GlobalItem g) => new Action<Item, BinaryWriter>(g.NetSend), (GlobalItem g) => new Action<Item, BinaryReader>(g.NetReceive));
		if (!IsCloneable)
		{
	...
	/// <returns></returns>
	public virtual bool? PrefixChance(Item item, int pre, UnifiedRandom rand)
	{
		return null;
	}
	...
	public virtual float UseAnimationMultiplier(Item item, Player player)
	{
		return 1f;
	}
	...
	public virtual float UseSpeedMultiplier(Item item, Player player)
	{
		return 1f;
	}
