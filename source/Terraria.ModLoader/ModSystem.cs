...
public abstract class ModSystem : ModType
{
	...
	{
		base.ValidateType();
		LoaderUtils.MustOverrideTogether(this, (ModSystem s) => new Action<TagCompound>(s.SaveWorldData), (ModSystem s) => new Action<TagCompound>(s.LoadWorldData));
		LoaderUtils.MustOverrideTogether(this, (ModSystem s) => new Action<BinaryWriter>(s.NetSend), (ModSystem s) => new Action<BinaryReader>(s.NetReceive));
	}
