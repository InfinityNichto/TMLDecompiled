...
public static class ContentInstance
{
	...
	private static ConcurrentDictionary<Type, ContentEntry> contentByType;

	static ContentInstance()
	{
		contentByType = new ConcurrentDictionary<Type, ContentEntry>();
		TypeCaching.OnClear += Clear;
	}

	private static ContentEntry Factory(Type t)
	{
	...
	{
		contentByType.GetOrAdd(t, Factory).Link(update);
	}

	public static void Register(object obj)
	{
		contentByType.GetOrAdd(obj.GetType(), Factory).Register(obj);
	}

	private static void Clear()
	{
		foreach (KeyValuePair<Type, ContentEntry> entry in contentByType)
		{
			entry.Value.Clear();
			if (entry.Key.Assembly != typeof(ContentEntry).Assembly)
			{
				contentByType.TryRemove(entry.Key, out var _);
			}
		}
	}
}
