...
public static class ContentSamples
{
	...
	public static class CreativeHelper
	{
		...
		public static void SetCreativeMenuOrder()
		{
			...
			IOrderedEnumerable<IGrouping<ItemGroup, ItemGroupAndOrderInGroup>> orderedEnumerable = from x in list
				...
				select @group;
			foreach (IGrouping<ItemGroup, ItemGroupAndOrderInGroup> item3 in orderedEnumerable)
			{
				foreach (ItemGroupAndOrderInGroup item2 in item3)
				{
					ItemCreativeSortingId[item2.ItemType] = item2;
				}
			}
	...
	public static class BestiaryHelper
	{
		public static List<KeyValuePair<int, NPC>> GetSortedBestiaryEntriesList(BestiaryDatabase database)
		{
			...
			List<KeyValuePair<int, NPC>> list = NpcsByNetId.ToList().OrderBy(delegate(KeyValuePair<int, NPC> x)
			{
				KeyValuePair<int, NPC> keyValuePair10 = x;
				return GetBestiaryTownPriority(keyValuePair10.Value);
			}).ThenBy(delegate(KeyValuePair<int, NPC> x)
			{
				KeyValuePair<int, NPC> keyValuePair9 = x;
				return !keyValuePair9.Value.isLikeATownNPC;
			})
				.ThenBy(delegate(KeyValuePair<int, NPC> x)
				{
					KeyValuePair<int, NPC> keyValuePair8 = x;
					return GetBestiaryNormalGoldCritterPriority(keyValuePair8.Value);
				})
				.ThenBy(delegate(KeyValuePair<int, NPC> x)
				{
					KeyValuePair<int, NPC> keyValuePair7 = x;
					return !keyValuePair7.Value.CountsAsACritter;
				})
				.ThenBy(delegate(KeyValuePair<int, NPC> x)
				{
					KeyValuePair<int, NPC> keyValuePair6 = x;
					return GetBestiaryBossPriority(keyValuePair6.Value);
				})
				.ThenBy(delegate(KeyValuePair<int, NPC> x)
				{
					KeyValuePair<int, NPC> keyValuePair5 = x;
					if (!keyValuePair5.Value.boss)
					{
						bool[] shouldBeCountedAsBoss = NPCID.Sets.ShouldBeCountedAsBoss;
						keyValuePair5 = x;
						return shouldBeCountedAsBoss[keyValuePair5.Value.type];
					}
					return true;
				})
				.ThenBy(delegate(KeyValuePair<int, NPC> x)
				{
					KeyValuePair<int, NPC> keyValuePair4 = x;
					return GetLowestBiomeGroupIndex(keyValuePair4.Value, database, commonFilters);
				})
				.ThenBy(delegate(KeyValuePair<int, NPC> x)
				{
					KeyValuePair<int, NPC> keyValuePair3 = x;
					return keyValuePair3.Value.aiStyle;
				})
				.ThenBy(delegate(KeyValuePair<int, NPC> x)
				{
					KeyValuePair<int, NPC> keyValuePair2 = x;
					return GetBestiaryPowerLevel(keyValuePair2.Value);
				})
				.ThenBy(delegate(KeyValuePair<int, NPC> x)
				{
					KeyValuePair<int, NPC> keyValuePair = x;
					return GetBestiaryStarsPriority(keyValuePair.Value);
				})
				.ToList();
