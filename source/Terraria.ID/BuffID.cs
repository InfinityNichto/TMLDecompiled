...
public class BuffID
{
	public class Sets
	{
		...
		/// <summary>
		/// If <see langword="true" /> for a given <see cref="T:Terraria.ID.BuffID" />, then that buff is not considered a typical debuff, but is a buff used to "tag" entities for other purposes.<br /><br /> 
		...
		/// Buffs in this set can be applied to all NPCs unless that NPC sets <see cref="F:Terraria.ID.NPCID.Sets.ImmuneToAllBuffs" /> to prevent it.<br /><br /> 
		/// Defaults to <see langword="false" />.
		/// </summary>
		public static bool[] IsATagBuff;
		...
		/// </remarks>
		public static bool[] LongerExpertDebuff;

		/// <summary>
		/// An NPC will automatically become immune (<see cref="F:Terraria.NPC.buffImmune" />) to the indexed buff type if it is already immune to any buff in the corresponding List. <br />
		/// For NPCs with <see cref="F:Terraria.ID.NPCID.Sets.ImmuneToAllBuffs" /> or <see cref="F:Terraria.ID.NPCID.Sets.ImmuneToRegularBuffs" /> set to true, the reverse of that logic is used to automatically set the NPC as vulnerable: The NPC will automatically become vulnerable to the indexed buff type if it is specifically vulnerable to all of the buffs in the corresponding List.<br /><br />
		/// This set helps standardize several buff immunity inheritance rules that are effectively present in Terraria while providing modders the ability to do the same. For example, all NPC with immunity to <see cref="F:Terraria.ID.BuffID.OnFire" /> are also immune to <see cref="F:Terraria.ID.BuffID.OnFire3" /> (aka, "On Fire!" and "Hellfire").<br />
		...
		/// </summary>
		public static List<int>[] GrantImmunityWith;

		static Sets()
		{
			Factory = new SetFactory(BuffLoader.BuffCount);
			IsWellFed = Factory.CreateBoolSet(26, 206, 207);
			IsFedState = Factory.CreateBoolSet(26, 206, 207, 332, 333, 334);
			IsATagBuff = Factory.CreateBoolSet(307, 313, 319, 316, 310, 309, 315, 326, 340);
			TimeLeftDoesNotDecrease = Factory.CreateBoolSet(28, 334, 29, 159, 150, 93, 348);
			CanBeRemovedByNetMessage = Factory.CreateBoolSet(313);
			IsAFlaskBuff = Factory.CreateBoolSet(71, 72, 73, 74, 75, 76, 77, 78, 79);
			NurseCannotRemoveDebuff = Factory.CreateBoolSet(28, 34, 87, 89, 21, 86, 199, 332, 333, 334, 165, 146, 48, 158, 157, 350, 215, 147);
			BasicMountData = Factory.CreateCustomSet<BuffMountData>(null, new object[108]
			{
				118,
				new BuffMountData
				{
			...
			LongerExpertDebuff = Factory.CreateBoolSet(20, 22, 23, 24, 30, 31, 32, 33, 35, 36, 39, 44, 46, 47, 69, 70, 80);
			GrantImmunityWith = Factory.CreateCustomSet<List<int>>(null, new object[10]
			{
				323,
				new List<int> { 24 },
				24,
			...
			{
				List<int>[] grantImmunityWith = GrantImmunityWith;
				int num = i;
				if (grantImmunityWith[num] == null)
				{
					grantImmunityWith[num] = new List<int>();
				}
			}
		}
	}
