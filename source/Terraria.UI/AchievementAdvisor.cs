...
public class AchievementAdvisor
{
	...
	private void DrawCard(AchievementAdvisorCard card, SpriteBatch spriteBatch, Vector2 position, float scale, out bool hovered)
	{
		...
		if (!hovered)
		{
			((Color)(ref color))._002Ector(220, 220, 220, 220);
		}
		...
		frame.X += 528;
		spriteBatch.Draw(_achievementsTexture.Value, position, (Rectangle?)frame, color, 0f, Vector2.Zero, scale, (SpriteEffects)0, 0f);
		spriteBatch.Draw(_achievementsBorderTexture.Value, position + vector, (Rectangle?)null, color, 0f, Vector2.Zero, scale, (SpriteEffects)0, 0f);
		if (hovered)
		{
			spriteBatch.Draw(value, position + vector2, (Rectangle?)null, Main.OurFavoriteColor, 0f, Vector2.Zero, scale, (SpriteEffects)0, 0f);
		}
	}
