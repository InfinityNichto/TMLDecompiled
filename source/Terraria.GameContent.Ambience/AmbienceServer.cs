...
public class AmbienceServer
{
	...
	{
		SpawnForcedEntities();
		if (_updatesUntilNextAttempt > 0.0)
		{
			_updatesUntilNextAttempt -= Main.desiredWorldEventsUpdateRate;
			return;
		}
		ResetSpawnTime();
		IEnumerable<SkyEntityType> source = _spawnConditions.Where(delegate(KeyValuePair<SkyEntityType, Func<bool>> pair)
		{
			KeyValuePair<SkyEntityType, Func<bool>> keyValuePair2 = pair;
