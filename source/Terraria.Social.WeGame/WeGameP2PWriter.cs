...
public class WeGameP2PWriter
{
	...
	public void QueueSend(RailID user, byte[] data, int length)
	{
		lock (_lock)
		{
			Queue<WriteInformation> queue5;
			if (!_pendingSendData.ContainsKey(user))
			{
				Queue<WriteInformation> queue4 = (_pendingSendData[user] = new Queue<WriteInformation>());
				queue5 = queue4;
			}
			else
			{
				queue5 = _pendingSendData[user];
			}
			Queue<WriteInformation> queue2 = queue5;
			int num = length;
			int num2 = 0;
			while (num > 0)
			{
	...
	public void SendAll()
	{
		...
		foreach (KeyValuePair<RailID, Queue<WriteInformation>> item in _pendingSendDataSwap)
		{
			...
			while (value.Count > 0)
			{
				...
				rail_api.RailFactory().RailNetworkHelper().SendData(GetLocalPeer(), item.Key, writeInformation.Data, (uint)writeInformation.Size);
