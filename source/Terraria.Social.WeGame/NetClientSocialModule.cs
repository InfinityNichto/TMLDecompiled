...
public class NetClientSocialModule : NetSocialModule
{
	...
	public override void LaunchLocalServer(Process process, ServerMode mode)
	{
		...
		rail_api.RailFactory().RailUtils().GetLaunchAppParameters((EnumRailLaunchAppType)2, ref parameter);
	...
	private void OnGetFriendMetaData(RailFriendsGetMetadataResult data)
	{
		...
		if (valueByKey.Length > 0)
		{
			...
			((RailComparableID)railID).id_ = ulong.Parse(valueByKey);
			if (((RailComparableID)railID).IsValid())
			{
				JoinServer(railID);
			}
			else
	...
	private void AsyncSetPlayWith(RailID rail_id)
	{
		...
		IRailFriends obj = rail_api.RailFactory().RailFriends();
		if (obj != null)
		{
			obj.AsyncReportPlayedWithUserList(list, "");
		}
	}
	...
	private void AsyncGetServerIDByOwener(RailID ownerID)
	{
		...
		if (obj != null)
		{
			obj.AsyncGetFriendMetadata(ownerID, list, "");
		}
	}
