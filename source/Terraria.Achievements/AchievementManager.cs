	...
	private void Save(string path, bool cloud)
	{
		...
		lock (_ioLock)
		{
			...
			try
			{
				...
				BsonWriter bsonWriter = new BsonWriter((Stream)cryptoStream);
				try
				{
					JsonSerializer.Create(_serializerSettings).Serialize((JsonWriter)(object)bsonWriter, (object)_achievements);
				...
				{
					((IDisposable)bsonWriter)?.Dispose();
				}
			}
	...
	private void Load(string path, bool cloud)
	{
		...
		lock (_ioLock)
		{
			...
			try
			{
				...
				BsonReader reader = new BsonReader((Stream)stream2);
				try
				{
					dictionary = JsonSerializer.Create(_serializerSettings).Deserialize<Dictionary<string, StoredAchievement>>((JsonReader)(object)reader);
				}
				finally
				{
