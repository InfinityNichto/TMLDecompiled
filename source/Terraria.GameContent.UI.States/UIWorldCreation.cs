...
public class UIWorldCreation : UIState
{
	...
	private void SetDefaultOptions()
	{
		...
		for (int l = 0; l < sizeButtons.Length; l++)
		{
			sizeButtons[l].SetCurrentOption(WorldSizeId.Medium);
		}
		...
		if (Main.ActivePlayerFileData.Player.difficulty == 3)
		{
			...
			{
				difficultyButtons[j].SetCurrentOption(WorldDifficultyId.Creative);
			}
		...
		{
			GroupOptionButton<WorldDifficultyId>[] difficultyButtons2 = _difficultyButtons;
			for (int k = 0; k < difficultyButtons2.Length; k++)
			{
				difficultyButtons2[k].SetCurrentOption(WorldDifficultyId.Normal);
			}
		}
	...
	private void AddWorldDifficultyOptions(UIElement container, float accumualtedHeight, MouseEvent clickEvent, string tagGroup, float usableWidthPercent)
	{
		...
		Color[] array4 = (Color[])(object)new Color[4]
		{
	...
	{
		GroupOptionButton<WorldSizeId>[] sizeButtons = _sizeButtons;
		for (int k = 0; k < sizeButtons.Length; k++)
		{
			sizeButtons[k].SetCurrentOption(_optionSize);
		}
		GroupOptionButton<WorldDifficultyId>[] difficultyButtons = _difficultyButtons;
		for (int j = 0; j < difficultyButtons.Length; j++)
		{
			difficultyButtons[j].SetCurrentOption(_optionDifficulty);
		}
		GroupOptionButton<WorldEvilId>[] evilButtons = _evilButtons;
		for (int i = 0; i < evilButtons.Length; i++)
		{
	...
	private void AssignRandomWorldName()
	{
		do
		{
			...
			LocalizedText localizedText4 = Language.SelectRandom(Lang.CreateDialogFilter("RandomWorldName_Adjective."));
			LocalizedText localizedText2 = Language.SelectRandom(Lang.CreateDialogFilter("RandomWorldName_Location."));
			LocalizedText localizedText3 = Language.SelectRandom(Lang.CreateDialogFilter("RandomWorldName_Noun."));
			var obj = new
			{
