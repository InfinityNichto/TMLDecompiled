...
public abstract class AWorkshopPublishInfoState<TPublishedObjectType> : UIState, IHaveBackButtonCommand
{
	...
	private void SetTagsFromFoundEntry()
	{
		...
		if (info.tags != null)
		{
			foreach (GroupOptionButton<WorkshopTagOption> tagOption in _tagOptions)
			{
				...
				tagOption.SetColor((Color)(tagOption.IsSelected ? new Color(152, 175, 235) : Colors.InventoryDefaultColor), 1f);
			}
		}
	...
	private void ClickTagOption(UIMouseEvent evt, UIElement listeningElement)
	{
		...
		groupOptionButton.SetColor((Color)(groupOptionButton.IsSelected ? new Color(152, 175, 235) : Colors.InventoryDefaultColor), 1f);
	}
	...
	private void UpdateImagePreview()
	{
		...
		if (_previewImagePath != null)
		{
			try
			{
				...
				texture2D = Texture2D.FromStream(Main.graphics.GraphicsDevice, (Stream)stream);
			}
		...
		else
		{
			...
			{
				((GraphicsResource)_previewImageTransientTexture).Dispose();
