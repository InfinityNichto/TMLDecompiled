...
public class UICreativeInfiniteItemsDisplay : UIElement
{
	...
	private void AddCogsForSacrificeMenu(UIElement sacrificesContainer)
	{
		...
		((Vector2)(ref vector))._002Ector(-10f, -10f);
	...
	private void UpdateVisualFrame()
	{
		...
		OffsetRotationsForCogs(1.1428572f * num, _sacrificeCogsMedium);
		OffsetRotationsForCogs((0f - num4) * num, _sacrificeCogsBig);
		int frameY = 0;
		if (_sacrificeAnimationTimeLeft != 0)
		{
			...
			if (_sacrificeAnimationTimeLeft == 56)
			{
				...
				Vector2 accelerationPerFrame = default(Vector2);
				((Vector2)(ref accelerationPerFrame))._002Ector(0f, 0.16350001f);
				for (int i = 0; i < 15; i++)
				{
	...
	{
		_ = UserInterface.ActiveInstance;
		if (Main.gameMenu)
		{
			return Main.MenuUI;
		}
