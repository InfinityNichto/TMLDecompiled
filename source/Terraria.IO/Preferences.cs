		...
		lock (_lock)
		{
			...
			try
			{
				...
				else
				{
					...
					BsonReader reader = new BsonReader((Stream)stream);
					try
					{
						JsonSerializer jsonSerializer = JsonSerializer.Create(_serializerSettings);
						_data = jsonSerializer.Deserialize<Dictionary<string, object>>((JsonReader)(object)reader);
					}
					finally
					{
		...
		lock (_lock)
		{
			try
			{
				...
				else
				{
					...
					BsonWriter jsonWriter = new BsonWriter((Stream)stream);
					try
					{
						File.SetAttributes(_path, FileAttributes.Normal);
						JsonSerializer.Create(_serializerSettings).Serialize((JsonWriter)(object)jsonWriter, (object)_data);
					}
					finally
					{
