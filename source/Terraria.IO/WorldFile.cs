...
public class WorldFile
{
	...
	public static WorldFileData GetAllMetadata(string file, bool cloudSave)
	{
		...
		{
			Logging.Terraria.Error((object)$"Error reading WorldFileData: {file}, IsCloudSave={cloudSave}", e);
		}
		return null;
	}
	...
	private static void InternalSaveWorld(bool useCloudSaving, bool resetTime)
	{
		...
		using (MemoryStream input = new MemoryStream(array, 0, num, writable: false))
		{
			...
			{
				Logging.Terraria.Error((object)"ValidateWorld failed. A bug in WorldFile.Save has produced an invalid file. Step through with a debugger to find out where ValidateWorld returns false.");
				validated = false;
	...
	private static void ConvertOldTileEntities()
	{
		...
		foreach (Point item2 in list2)
		{
			...
			short frameX5 = Main.tile[item2.X, item2.Y + 1].frameX;
			int frameX4 = Main.tile[item2.X + 1, item2.Y + 1].frameX;
			bool flag2 = frameX5 >= 5000;
			int num2 = frameX5 % 5000;
			num2 -= 100;
	...
	public static int SaveChests(BinaryWriter writer)
	{
		...
		for (int j = 0; j < 8000; j++)
		{
			...
			bool flag = false;
			for (int k = chest2.x; k <= chest2.x + 1; k++)
			{
				for (int l = chest2.y; l <= chest2.y + 1; l++)
				{
					if (k < 0 || l < 0 || k >= Main.maxTilesX || l >= Main.maxTilesY)
					{
						flag = true;
						break;
					}
	...
	public static void LoadNPCs(BinaryReader reader)
	{
		...
		while (flag)
		{
			...
			{
				nPC2.SetDefaults(reader.ReadInt32());
			}
			else
			{
				nPC2.SetDefaults(NPCID.FromLegacyName(reader.ReadString()));
			}
			nPC2.GivenName = reader.ReadString();
			nPC2.position.X = reader.ReadSingle();
			nPC2.position.Y = reader.ReadSingle();
			nPC2.homeless = reader.ReadBoolean();
			nPC2.homeTileX = reader.ReadInt32();
			nPC2.homeTileY = reader.ReadInt32();
			if (_versionNumber >= 213 && ((BitsByte)reader.ReadByte())[0])
			{
				nPC2.townNpcVariationIndex = reader.ReadInt32();
			}
			num2++;
	...
	public static bool ValidateWorld(BinaryReader fileIO)
	{
		...
		try
		{
			...
			{
				float num13 = (float)i / (float)Main.maxTilesX;
				Main.statusText = Lang.gen[73].Value + " " + (int)(num13 * 100f + 1f) + "%";
				int num14;
				for (num14 = 0; num14 < num11; num14++)
				{
					byte b;
					byte b2 = (b = 0);
			...
			if (_versionNumber >= 189)
			{
				int num8 = fileIO.ReadInt32();
				fileIO.ReadBytes(12 * num8);
			}
			if (_versionNumber >= 210)
			{
			...
			bool result = false;
			if (num17 && (text2 == text || num9 == num10))
			{
				result = true;
			}
			return result;
		...
		{
			Logging.Terraria.Error((object)"World Validation", value);
	...
	public static int SaveTileEntities(BinaryWriter writer)
	{
		lock (TileEntity.EntityCreationLock)
		{
			...
			foreach (KeyValuePair<int, TileEntity> item in TileEntity.ByID.Where((KeyValuePair<int, TileEntity> pair) => pair.Value.type < ModTileEntity.NumVanilla))
			{
	...
	private static int LoadWorld_Version1_Old_BeforeRelease88(BinaryReader fileIO)
	{
		...
		if (versionNumber >= 60)
		{
			...
			style9 = fileIO.ReadByte();
		}
		WorldGen.setBG(0, style);
		WorldGen.setBG(1, style3);
		WorldGen.setBG(2, style4);
		WorldGen.setBG(3, style5);
		WorldGen.setBG(4, style6);
		WorldGen.setBG(5, style7);
		WorldGen.setBG(6, style8);
		WorldGen.setBG(7, style9);
		WorldGen.setBG(8, style10);
		WorldGen.setBG(9, style2);
		...
		for (int i = 0; i < Main.maxTilesX; i++)
		{
			...
			for (int j = 0; j < Main.maxTilesY; j++)
			{
				...
				if (tile.active())
				{
					num5 = ((versionNumber <= 77) ? fileIO.ReadByte() : fileIO.ReadUInt16());
					...
					else if (Main.tileFrameImportant[num5])
					{
						if (versionNumber < 28 && num5 == 4)
						{
							tile.frameX = 0;
							tile.frameY = 0;
						}
				...
				if (versionNumber >= 25)
				{
					num6 = fileIO.ReadInt16();
				}
				if (num5 != -1)
				{
					if ((double)j <= Main.worldSurface)
					{
						if ((double)(j + num6) <= Main.worldSurface)
						{
							WorldGen.tileCounts[num5] += (num6 + 1) * 5;
						}
						else
						{
							int num7 = (int)(Main.worldSurface - (double)j + 1.0);
							int num8 = num6 + 1 - num7;
							WorldGen.tileCounts[num5] += num7 * 5 + num8;
						}
					}
					else
					{
						WorldGen.tileCounts[num5] += num6 + 1;
					}
				}
				if (num6 > 0)
				{
					for (int k = j + 1; k < j + num6 + 1; k++)
					{
		...
		for (int l = 0; l < num10; l++)
		{
			...
			for (int m = 0; m < 40; m++)
			{
				...
				int num11 = 0;
				num11 = ((versionNumber < 59) ? fileIO.ReadByte() : fileIO.ReadInt16());
				if (num11 > 0)
				{
					if (versionNumber >= 38)
					{
					...
					Main.chest[l].item[m].stack = num11;
					if (versionNumber >= 36)
					{
		...
		for (int n = 0; n < 1000; n++)
		{
			if (fileIO.ReadBoolean())
			{
				...
				if (Main.tile[num12, num2].active() && (Main.tile[num12, num2].type == 55 || Main.tile[num12, num2].type == 85))
				{
		...
		if (versionNumber >= 7)
		{
			bool num13 = fileIO.ReadBoolean();
			string text3 = fileIO.ReadString();
			int num4 = fileIO.ReadInt32();
			if (num13 && (text3 == Main.worldName || num4 == Main.worldID))
			{
				return 0;
			}
			return 2;
