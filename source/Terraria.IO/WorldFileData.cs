...
public class WorldFileData : FileData
{
	...
	/// <summary>
	/// If <see langword="true" />, the mods used to genereate this world have been saved and their version can be retrieved using <see cref="M:Terraria.IO.WorldFileData.TryGetModVersionGeneratedWith(System.String,System.Version@)" />.<para />
	...
	{
		return modVersionsDuringWorldGen.TryGetValue(mod, out modVersion);
	}

	public bool TryGetHeaderData<T>(out TagCompound data) where T : ModSystem
	{
		return TryGetHeaderData(ModContent.GetInstance<T>(), out data);
	}

	public bool TryGetHeaderData(ModSystem system, out TagCompound data)
	{
		return ModHeaders.TryGetValue(system.FullName, out data);
	}
}
