...
internal static class PlayerIO
{
	...
	{
		writer.Write((byte)((hairDye <= EffectsTracker.vanillaHairShaderCount) ? ((uint)hairDye) : 0u));
	}

	internal static void WriteVanillaHair(int hair, BinaryWriter writer)
	{
		writer.Write((hair < HairID.Count) ? hair : 0);
	}
	...
	public static void LoadResearch(Player player, IList<TagCompound> list)
	{
		foreach (TagCompound tag in list)
		{
			if (tag.ContainsKey("mod") && tag.ContainsKey("name"))
			{
				...
				if (ModContent.TryFind<ModItem>(@string, modItemName, out var modItem))
				{
	...
	{
		if (!(hairDyeItemName == "") && ModContent.TryFind<ModItem>(hairDyeItemName, out var modItem))
		{
	...
	{
		if (!(hairName == "") && ModContent.TryFind<ModHair>(hairName, out var modHair))
		{
	...
	internal static void LoadModData(Player player, IList<TagCompound> list)
	{
		foreach (TagCompound tag in list)
		{
			...
			if (ModContent.TryFind<ModPlayer>(@string, modPlayerName, out var modPlayerBase))
			{
				ModPlayer modPlayer = player.GetModPlayer(modPlayerBase);
				try
				{
	...
	internal static void LoadModBuffs(Player player, IList<TagCompound> list)
	{
		...
		foreach (TagCompound tag2 in list.Reverse())
		{
			if (ModContent.TryFind<ModBuff>(tag2.GetString("mod"), tag2.GetString("name"), out var buff2))
			{
				int index = Math.Min(tag2.GetByte("index"), buffCount);
	...
	internal static List<TagCompound> SaveBuilderToggles(Player player)
	{
		return (from x in BuilderToggleLoader.BuilderToggles
			...
			select new TagCompound
			{
				["fullName"] = x.FullName,
				["currentState"] = player.builderAccStatus[x.Type]
			}).ToList();
	}
