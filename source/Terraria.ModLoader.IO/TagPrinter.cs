...
public class TagPrinter
{
	...
	private void WriteValue(object elem)
	{
		...
		else if (elem is TagCompound)
		{
			...
			WriteList('{', '}', multiline: true, tag, WriteEntry);
		}
		else
		{
			if (!(elem is IList))
			...
			WriteList('[', ']', type == typeof(string) || type == typeof(TagCompound) || typeof(IList).IsAssignableFrom(type), ((IList)elem).Cast<object>(), delegate(object o)
			{
				if (type == typeof(IList))
				{
