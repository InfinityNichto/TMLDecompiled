...
public class DD2Film : Film
{
	...
	private void OgreSwingSound(FrameEventData evt)
	{
		...
		SoundEngine.PlaySound(in SoundID.DD2_OgreAttack, _ogre.Center);
	}
	...
	private void DryadPortalFade(FrameEventData evt)
	{
		...
		if (evt.IsFirstFrame)
		{
			SoundEngine.PlaySound(in SoundID.DD2_EtherianPortalDryadTouch, _dryad.Center);
		}
		...
		int num = (int)(6f * val);
		Vector2 size = _dryad.Size;
		float num2 = ((Vector2)(ref size)).Length() / 2f;
		num2 /= 20f;
		for (int i = 0; i < num; i++)
		{
	...
	private void DryadConfusedEmote(FrameEventData evt)
	{
		...
		{
			EmoteBubble.NewBubble(87, new WorldUIAnchor((Entity)_dryad), evt.Duration);
		}
	}
	...
	private void DryadAlertEmote(FrameEventData evt)
	{
		if (_dryad != null && evt.IsFirstFrame)
		{
			EmoteBubble.NewBubble(3, new WorldUIAnchor((Entity)_dryad), evt.Duration);
		}
	}
	...
	public override void OnBegin()
	{
		...
		Main.dayTime = true;
		Main.time = 27000.0;
		_startPoint = Main.screenPosition + new Vector2((float)Main.mouseX, (float)Main.mouseY - 32f);
		base.OnBegin();
	}
