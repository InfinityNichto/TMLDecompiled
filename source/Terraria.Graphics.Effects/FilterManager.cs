...
public class FilterManager : EffectManager<Filter>
{
	...
	public void BeginCapture(RenderTarget2D screenTarget1, Color clearColor)
	{
		...
		((Game)Main.instance).GraphicsDevice.Clear(clearColor);
	}
	...
	public void EndCapture(RenderTarget2D finalTexture, RenderTarget2D screenTarget1, RenderTarget2D screenTarget2, Color clearColor)
	{
		...
		if (Main.player[Main.myPlayer].gravDir == -1f)
		{
			...
			Main.spriteBatch.Begin((SpriteSortMode)1, BlendState.AlphaBlend, SamplerState.LinearClamp, DepthStencilState.Default, RasterizerState.CullNone, (Effect)null, Matrix.Invert(Main.GameViewMatrix.EffectMatrix));
			Main.spriteBatch.Draw((Texture2D)(object)renderTarget2D2, Vector2.Zero, Color.White);
			...
			if (value.Priority >= _priorityThreshold)
			{
				...
				if (num > _activeFilterCount - _filterLimit && value.IsVisible())
				{
					if (filter != null)
					{
						...
						filter.Apply();
						Main.spriteBatch.Draw((Texture2D)(object)renderTarget2D2, Vector2.Zero, Main.ColorOfTheSkies);
						Main.spriteBatch.End();
