...
internal class MayneWingLayer : PlayerDrawLayer
{
	...
	protected override void Draw(ref PlayerDrawSet drawInfo)
	{
		...
		{
			DrawData glow = new DrawData(ModContent.Request<Texture2D>("ModLoader/Patreon.Mayne_Wings_Wings_Glow", AssetRequestMode.ImmediateLoad).Value, color: Color.White * drawInfo.stealth * (1f - drawInfo.shadow), position: wingData.Value.position, sourceRect: wingData.Value.sourceRect, rotation: wingData.Value.rotation, origin: wingData.Value.origin, scale: wingData.Value.scale, effect: wingData.Value.effect);
