...
public class Cloud
{
	...
	public static void addCloud()
	{
		...
		Main.cloud[num].spriteDir = (SpriteEffects)0;
		if (rand.Next(2) == 0)
		{
			Main.cloud[num].spriteDir = (SpriteEffects)1;
		}
		...
		((Rectangle)(ref rectangle))._002Ector((int)Main.cloud[num].position.X, (int)Main.cloud[num].position.Y, Main.cloud[num].width, Main.cloud[num].height);
		Rectangle value = default(Rectangle);
		for (int j = 0; j < 200; j++)
		{
			if (num != j && Main.cloud[j].active)
			{
				((Rectangle)(ref value))._002Ector((int)Main.cloud[j].position.X, (int)Main.cloud[j].position.Y, Main.cloud[j].width, Main.cloud[j].height);
	...
	public Color cloudColor(Color bgColor)
	{
		...
		float num5 = (int)((float)(int)((Color)(ref bgColor)).R * num);
		float num2 = (int)((float)(int)((Color)(ref bgColor)).G * num);
		float num3 = (int)((float)(int)((Color)(ref bgColor)).B * num);
		float num4 = (int)((float)(int)((Color)(ref bgColor)).A * num);
		return new Color((int)(byte)num5, (int)(byte)num2, (int)(byte)num3, (int)(byte)num4);
	}
