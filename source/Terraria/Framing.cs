...
public class Framing
{
	...
	public static void SelfFrame8Way(int i, int j, Tile centerTile, bool resetFrame)
	{
		...
		BlockStyle blockStyle10 = FindBlockStyle(centerTile);
		int num = 0;
	...
	public static void WallFrame(int i, int j, bool resetFrame = false)
	{
		...
		if (j - 1 >= 0)
		{
			...
			if (tile5 != null && (tile5.wall > 0 || (tile5.active() && TileID.Sets.WallsMergeWith[tile5.type])) && (flag || !tile5.invisibleWall()))
			{
				num = 1;
			}
		}
		if (i - 1 >= 0)
		{
			...
			if (tile4 != null && (tile4.wall > 0 || (tile4.active() && TileID.Sets.WallsMergeWith[tile4.type])) && (flag || !tile4.invisibleWall()))
			{
				num |= 2;
			}
		}
		if (i + 1 <= Main.maxTilesX - 1)
		{
			...
			if (tile3 != null && (tile3.wall > 0 || (tile3.active() && TileID.Sets.WallsMergeWith[tile3.type])) && (flag || !tile3.invisibleWall()))
			{
				num |= 4;
			}
		}
		if (j + 1 <= Main.maxTilesY - 1)
		{
			...
			if (tile2 != null && (tile2.wall > 0 || (tile2.active() && TileID.Sets.WallsMergeWith[tile2.type])) && (flag || !tile2.invisibleWall()))
			{
				num |= 8;
			}
		}
