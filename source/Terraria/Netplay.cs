...
public class Netplay
{
	...
	private static void ClientLoopSetup(RemoteAddress address)
	{
		...
		Logging.Terraria.InfoFormat("Connecting to {0}", (object)friendlyName);
		Connection.ResetSpecialFlags();
		ResetNetDiag();
		...
		Connection.ReadBuffer = new byte[65535];
	}
	...
	public static bool SetRemoteIP(string remoteAddress)
	{
		if (remoteAddress.Contains(':'))
		{
			...
			if (int.TryParse(remoteAddress.AsSpan(colonInd + 1), out var port))
			{
				remoteAddress = remoteAddress.Substring(0, colonInd);
				bool num = SetRemoteIPOld(remoteAddress);
				if (num)
				{
					ListenPort = port;
				}
