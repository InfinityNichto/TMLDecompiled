...
public class ItemsSacrificedUnlocksTracker : IPersistentPerWorldContent, IOnPlayerJoining
{
	...
	public void Save(BinaryWriter writer)
	{
		int value;
		Item value2;
		Dictionary<string, int> dictionary = _sacrificeCountByItemPersistentId.Where((KeyValuePair<string, int> pair) => ContentSamples.ItemNetIdsByPersistentIds.TryGetValue(pair.Key, out value) && ContentSamples.ItemsByType.TryGetValue(value, out value2) && value2.ModItem == null).ToDictionary((KeyValuePair<string, int> pair) => pair.Key, (KeyValuePair<string, int> pair) => pair.Value);
