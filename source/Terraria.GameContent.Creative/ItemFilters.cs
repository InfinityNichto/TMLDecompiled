...
public static class ItemFilters
{
	public class BySearch : IItemEntryFilter, IEntryFilter<Item>, ISearchFilter<Item>
	{
		...
		public bool FitsFilter(Item entry)
		{
			...
			Main.MouseText_DrawItemTooltip_GetLinesInfo(entry, ref _unusedYoyoLogo, ref _unusedResearchLine, knockBack, ref numLines, _toolTipLines, _unusedPrefixLine, _unusedBadPrefixLines, _toolTipNames, out var _);
			Color?[] overrideColor;
			foreach (TooltipLine item in ItemLoader.ModifyTooltips(entry, ref numLines, _toolTipNames, ref _toolTipLines, ref _unusedPrefixLine, ref _unusedBadPrefixLines, ref _unusedYoyoLogo, out overrideColor, -1))
			{
				if (item.Text.ToLower().IndexOf(_search, StringComparison.OrdinalIgnoreCase) != -1)
				{
					return true;
				}
			}
