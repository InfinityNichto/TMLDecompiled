...
public class CoinLossRevengeSystem
{
	public class RevengeMarker
	{
		...
		public bool Intersects(Rectangle rectInner, Rectangle rectOuter)
		{
			...
			return ((Rectangle)(ref rectOuter)).Intersects(_hitbox);
		}
		...
		public bool DrawMapIcon(SpriteBatch spriteBatch, Vector2 mapTopLeft, Vector2 mapX2Y2AndOff, Rectangle? mapRect, float mapScale, float drawScale, int gameTime)
		{
			...
			{
				val = mapRect.Value;
				if (!((Rectangle)(ref val)).Contains(vector.ToPoint()))
				{
					return false;
				}
			}
			...
			spriteBatch.Draw(value, vector, (Rectangle?)rectangle, Color.White, 0f, rectangle.Size() / 2f, drawScale, (SpriteEffects)0, 0f);
			val = Utils.CenteredRectangle(vector, rectangle.Size() * drawScale);
			return ((Rectangle)(ref val)).Contains(Main.MouseScreen.ToPoint());
		}
	...
	public void CheckRespawns()
	{
		...
		for (int i = 0; i < 255; i++)
		{
			...
			{
				list.Add(Tuple.Create<int, Rectangle, Rectangle>(i, Utils.CenteredRectangle(player.Center, _playerBoxSizeInner), Utils.CenteredRectangle(player.Center, _playerBoxSizeOuter)));
			}
		}
		if (list.Count == 0)
		{
