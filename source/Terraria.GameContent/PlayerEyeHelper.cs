...
public struct PlayerEyeHelper
{
	...
	private void UpdateEyeFrameToShow(Player player)
	{
		...
		case EyeState.NormalBlinking:
		{
			...
			eyeFrameToShow = ((num >= 4) ? EyeFrame.EyeHalfClosed : ((num >= 2) ? EyeFrame.EyeClosed : ((num >= 0) ? EyeFrame.EyeHalfClosed : EyeFrame.EyeOpen)));
			break;
		}
		...
		case EyeState.InBed:
		{
			...
			eyeFrameToShow = ((_timeInState < 60) ? eyeFrame : ((_timeInState < 120) ? EyeFrame.EyeHalfClosed : EyeFrame.EyeClosed));
			break;
		}
