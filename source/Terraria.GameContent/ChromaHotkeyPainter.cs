...
public class ChromaHotkeyPainter
{
	...
	private class PaintKey
	{
		...
		public PaintKey(string triggerName, List<string> keys)
		{
			...
			{
				if (Enum.TryParse<Keys>(key, ignoreCase: true, out Keys result))
				{
					_xnaKeys.Add(result);
				}
			}
	...
	{
		foreach (KeyValuePair<string, PaintKey> key4 in _keys)
		{
			key4.Value.Unbind();
		}
		_keys.Clear();
		...
		foreach (KeyValuePair<string, PaintKey> key5 in _keys)
		{
			key5.Value.Bind();
		}
